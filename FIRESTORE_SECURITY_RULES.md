...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // Property sale marketplace listings
    match /propertySales/{property} {
      allow read: if true;
    ...
    // Public SEO guide content generated by the admin SEO agent
    match /seoContent/{content} {
      allow read: if resource.data.status == "published" || isAdmin();
      allow create, update, delete: if isAdmin();
    }

    // Gemini SEO agent run history
    match /marketingAgentRuns/{run} {
...
### Property Sale Collection
- ✅ **`propertySales` read**: Public, so land and apartment sale listings can appear on the website
- ✅ **`propertySales` create**: Authenticated user only, and `createdBy` must equal their UID
- ✅ **`propertySales` update/delete**: Listing owner or admin only
- ✅ **Supported listing types**: Land for Sale and Apartment Sale

### SEO Agent Collections
- ✅ **`seoContent` read**: Public only when `status == "published"`; admin can read drafts
- ✅ **`seoContent` write**: Admin only
- ✅ **`marketingAgentRuns`**: Admin only for read/write history
- ✅ **Gemini API key**: Never stored in Firestore or React; use the Firebase Functions `GEMINI_API_KEY` secret
